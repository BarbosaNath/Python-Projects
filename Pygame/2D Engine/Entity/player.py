...
class Player(Person):
    ...
    def set_state(self,
        ...
        if change_state: self.state[state] = on_off
        if change_anim : self.current_animation = state if on_off == True else "idle"

    def set_collide_box(self, offset):
        self.collide_box[2] = (self.sprite_sheet.sprite_size[0] - offset[0]) * self.scale[0]
        self.collide_box[3] = (self.sprite_sheet.sprite_size[1] - offset[1]) * self.scale[1]
    ...
    def update(self):
        ...
        else: self.set_state("walking", False)
